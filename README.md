...
SportsScope is an application created by Samantha Pothitakis, Taylor Micetich, Henry Leaders, and Alan Chen. This app was created to help local sports leagues keep track of players on different teams and games between two teams. There are two levels of user roles. When a user is not logged into the app, they can only view the teams created and updated by other users logged into the app. When a user is logged in, they can view teams made by other users and create, edit, and delete their own teams. Users logged in can create their own games and edit theirs and other users games. This is done so that is a user from the opposite team of the creator of the game wants to update information about the game, they can do so. If a user who is not logged in tries to create a team, they will be redirected to the login page. These user roles are the same for viewing, adding, and deleting teams. Any user can download a csv of players when looking at a specific team. The users that will create login credentials are meant to be coaches who can upload teams with their players. The users without login credentials are meant to be players who can view other teams and look at game times. 

SportsScope uses MongoDB to store data for user login, teams, games, and players. Each of these are stored in a different collection that is updated whenever a logged in user creates, updates, or deletes a team or when they create or delete a game. Players are kept in the database in MongoDB even when a team is deleted so they are considered free agents that anyone with access to the database can look at and create a team with.
...
<img width ="300" alt = "editscreen" src = "angular/app/games/PREEDITSCREEN.png">
